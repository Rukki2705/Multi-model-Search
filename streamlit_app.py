...
if st.session_state.page == "audio":
    ...
    if query_text and df is not None and st.button("Search Similar Audio"):
        try:
            search_similar_audios(query_text, df)
        except Exception as e:
            st.error(f"Error searching similar audio: {e}")
    elif query_text:
        st.error("Please upload a Parquet file containing audio data before searching.")
